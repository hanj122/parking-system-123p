document.addEventListener('DOMContentLoaded', () => {
    ...
    function renderFloors(capacity) {
        ...
        [1, 2, 3].forEach(floor => {
            ...
            const ringColor = pct >= 90 ? 'border-red-500' : pct >= 60 ? 'border-amber-400' : 'border-emerald-500';

            const card = document.createElement('div');
            card.className = 'bg-white rounded-apple-2xl shadow-sm p-4 sm:p-5 flex items-center justify-between gap-4 border border-black/5 hover:border-black/10 transition-colors';
            card.innerHTML = `
    ...
    function renderTickets(tickets) {
        ...
        tickets.forEach(t => {
            const card = document.createElement('div');
            card.className = 'bg-white rounded-apple-2xl shadow-sm p-4 sm:p-5 flex flex-col sm:flex-row sm:items-center justify-between gap-3 border border-black/5 hover:border-black/10 transition-colors';
            card.innerHTML = `
    ...
    if (btnEntry) {
        btnEntry.addEventListener('click', async (e) => {
            ...
            const entryTime = (simEntryTime && simEntryTime.value) 
            ...
            try {
                ...
                if (!res.ok) {
                    ...
                    showToast(`✓ Car entered! Assigned Slot <strong>${data.slotId}</strong> (Floor ${data.floor}) — Ticket #<strong>${data.ticketId}</strong>`);
                ...
                showToast('⚠️ Could not connect to backend server. Make sure node server is running on port 3000.', true);
            } finally {
    ...
    function showToast(msg, isError = false) {
        ...
        if (isError) {
            entryToast.className = 'flex items-center gap-3 rounded-apple-xl px-5 py-3.5 shadow-sm text-sm sm:text-[15px] bg-red-50 text-red-700 border border-red-200 mb-6';
            entryToast.children[0].textContent = '⚠️';
        } else {
            entryToast.className = 'flex items-center gap-3 rounded-apple-xl px-5 py-3.5 shadow-sm text-sm sm:text-[15px] bg-emerald-50 text-emerald-900 border border-emerald-200 mb-6';
    ...
    window.openCheckout = async function(ticketId, slotId, floor, entryTime) {
        ...
        if (coEntryTime) coEntryTime.textContent = new Date(entryTime).toLocaleString();
    ...
    if (btnPay) {
        btnPay.addEventListener('click', async () => {
            const exitTime       = (simExitTime && simExitTime.value) ? new Date(simExitTime.value).toISOString() : null;
            ...
            try {
                ...
                if (data.status === 'towed') {
                    ...
                    if (coBreakdown) coBreakdown.innerHTML = '<p class="text-[#8e8e93] text-xs">No fee collected — vehicle was marked as towed (>24h stay).</p>';
                } else {
                    if (coChange) coChange.textContent = data.changeGiven;

                    if (data.changeGiven === 0) {
                        if (coBreakdown) coBreakdown.innerHTML = '<p class="text-[#6e6e73] text-xs mt-1">Exact payment received — no change needed.</p>';
                    } else {
